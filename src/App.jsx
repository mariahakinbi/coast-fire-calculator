...
/** External links */
const AFFILIATE_URL =
  "https://app.facet.com/invited?utm_content=6637&utm_source=member-referral&utm_term=Mariah";
...
const BOOKS = [
  {
    ...
    author: "Dave Ramsey",
    who: "If you are in debt",
    link: "https://amzn.to/3OXs60W",
    emoji: "💸",
    coverColor: "#8B6914",
    cover: "/books/total_money_makeover.jpg",
  },
  {
    title: "Money for Couples",
    author: "Ramit Sethi",
    who: "If you need a money system",
    link: "https://amzn.to/4ssjXQS",
    emoji: "💑",
    coverColor: "#7c3aed",
    cover: "/books/money_for_couples.jpg",
  },
  {
    title: "I Will Teach You To Be Rich",
    author: "Ramit Sethi",
    who: "If you need a money system",
    link: "https://amzn.to/4l2D5SI",
    emoji: "💰",
    coverColor: "#1a1a2e",
    cover: "/books/i_will_teach.jpg",
  },
  {
    title: "The Psychology of Money",
    author: "Morgan Housel",
    who: "If you are struggling with money mindset",
    link: "https://amzn.to/4rOiUL1",
    emoji: "🧠",
    coverColor: "#f5f0e8",
    cover: "/books/psychology_of_money.jpg",
  },
];
...
function InfoTip({ text }) {
  ...
  /* Close tooltip when user interacts elsewhere on the page */
  useEffect(() => {
    if (!open) return;
    ...
    /* Close on any touch/click outside the tooltip */
...
function ChartTooltip({ active, payload, label }) {
  ...
  return (
    ...
    >
      <div style={{ fontSize: 11, color: "#a5b4fc", marginBottom: 5 }}>
        Age {label}
      </div>
      {payload.map(
        (p, i) =>
...
function FeedbackWidget() {
  ...
  const handleVote = (rating) => {
    setVoted(rating);
    trackEvent("feedback_vote", { rating });
  };
  const handleSubmit = () => {
    if (text.trim())
      trackEvent("feedback_suggestion", { text: text.trim(), rating: voted });
    setDone(true);
  };
        ...
        <>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <input
              ...
              placeholder="e.g., tax modeling, Roth vs 401k..."
  ...
  return (
    <div
      style={{
        minHeight: "100vh",
        background:
          "linear-gradient(180deg, #ede9fe 0%, #f3e8ff 25%, #faf5ff 55%, #fff 100%)",
        fontFamily: "'Inter',sans-serif",
      }}
    >
      <link
      ...
      />
      <div
        style={{ padding: "40px 20px 32px", maxWidth: 500, margin: "0 auto" }}
      >
        <button
          onClick={onBack}
          style={{
        ...
        <div style={{ fontSize: 38, marginBottom: 6, textAlign: "center" }}>
          📚
        </div>
        <h1
          style={{
          ...
          >
            <div
              style={{
                ...
                border: "1px solid #f3f4f6",
                boxShadow:
                  "0 1px 4px rgba(0,0,0,0.03), 0 4px 16px rgba(0,0,0,0.02)",
...
export default function CoastFIRECalculator() {
          ...
          balance_range:
            inputs.balance < 50000
              ...
              : inputs.balance < 100000
                ? "50-100K"
                : inputs.balance < 250000
                  ? "100-250K"
                  : inputs.balance < 500000
                    ? "250-500K"
                    : inputs.balance < 1000000
                      ? "500K-1M"
                      : "1M+",
          monthly: inputs.monthlyContrib,
          retire_age: inputs.retireAge,
          spending: inputs.annualSpending,
          return_pct: inputs.annualReturn,
          inflation_pct: inputs.inflation,
          swr_pct: inputs.swr,
        });
      }
    }, 5000);
  ...
  const results = useMemo(() => {
    ...
    const coastNumber =
      yearsToRetire > 0 && realReturn > 0
        ? retirementTarget / Math.pow(1 + realReturn, yearsToRetire)
        : retirementTarget;
    ...
    for (let age = currentAge; age <= 100; age++) {
      const coastTarget =
        age <= retireAge
          ? retirementTarget /
            Math.pow(1 + realReturn, Math.max(0, retireAge - age))
          : null;
      ...
      rows.push({
        age,
        keepInvesting: Math.max(0, keepBalance),
        coast: Math.max(0, coastBalance),
        target: coastTarget,
        withdrawal: age > retireAge ? annualSpending : null,
        status,
      });

      if (age > retireAge && coastBalance <= 0 && moneyLastsAge === null)
        moneyLastsAge = age - 1;

      if (age < 100) {
        if (age < retireAge) {
          keepBalance = keepBalance * (1 + realReturn) + annualContrib;
          coastBalance = coastReached
            ? coastBalance * (1 + realReturn)
            : coastBalance * (1 + realReturn) + annualContrib;
        } else {
          keepBalance = Math.max(
            0,
            keepBalance * (1 + realReturn) - annualSpending,
          );
          coastBalance = Math.max(
            0,
            coastBalance * (1 + realReturn) - annualSpending,
          );
        }
      }
    }
    ...
    const keepProjection =
      rows.find((r) => r.age === retireAge)?.keepInvesting || 0;
    ...
    if (!onTrack && realReturn > 0 && yearsToRetire > 0) {
      const annuityFactor =
        (Math.pow(1 + realReturn, yearsToRetire) - 1) / realReturn;
      if (annuityFactor > 0)
        extraMonthly = Math.abs(surplus) / annuityFactor / 12;
    }

    let coastMessage;
    if (balance >= coastNumber)
      coastMessage = {
        text: "You can stop contributing now!",
        type: "success",
      };
    else if (coastAge !== null && coastAge > currentAge)
      coastMessage = {
        text: `You can stop contributing at age ${coastAge}`,
        type: "success",
      };
    else
      coastMessage = {
        text: `No coast period. Keep investing until age ${retireAge}.`,
        type: "warning",
      };
    ...
    return {
      ...
      retirementTarget,
      coastNumber,
      alreadyCoast: balance >= coastNumber,
      coastAge,
      coastMessage,
      noCoast: coastAge === null,
      coastProjection,
      keepProjection,
      onTrack,
      surplus,
      ...
      hasShortfallButLasts: surplus < 0 && moneyLastsAge === null,
      rows,
    };
  }, [inputs]);
  ...
  if (page === "books")
    return <BooksPage onBack={() => setPage("calculator")} />;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <p
          style={{ fontSize: 13, color: "#7c3aed", fontWeight: 600, margin: 0 }}
        >
        ...
        <button
          onClick={() => {
            ...
            trackEvent("toggle_whatis", { open: !showInfo });
          }}
          style={{
            marginTop: 10,
            background: showInfo ? "#7c3aed" : "rgba(139,92,246,0.08)",
            border: "1px solid rgba(139,92,246,0.2)",
            borderRadius: 20,
            padding: "7px 18px",
            color: showInfo ? "#fff" : "#7c3aed",
            fontSize: 12,
            fontWeight: 600,
            cursor: "pointer",
          }}
        ...
        {showInfo && (
          ...
          >
            ...
            >
              <strong style={{ color: "#7c3aed" }}>Coast FIRE</strong> is the
              point where your investments are large enough that compound growth
              alone will carry you to your retirement goal, even if you never
              save another dollar.
            </p>
          </div>
        )}
      ...
      <div style={{ padding: "0 20px", maxWidth: 500, margin: "0 auto" }}>
        <div
          style={{
            background:
              results.coastMessage.type === "success"
            ...
            boxShadow:
              results.coastMessage.type === "success"
                ...
                : "0 8px 32px rgba(124,45,18,0.25)",
          }}
        >
          <div
            style={{
              ...
              fontWeight: 700,
              color:
                results.coastMessage.type === "success" ? "#a5b4fc" : "#fdba74",
            }}
          >
            🔥 Coast FIRE
          </div>
          ...
          >
            ...
            >
              {results.onTrack
                ...
                : `⚠️ ${formatCurrency(results.extraMonthly)}/mo more needed`}
            </span>
          </div>
        </div>
      </div>
      ...
      {/* ── Main Content ── */}
      <div
        style={{ padding: "20px 20px 32px", maxWidth: 500, margin: "0 auto" }}
      >
        {/* ── Input Card ── */}
        <div
          style={{
            ...
            marginBottom: 16,
            boxShadow:
              "0 1px 4px rgba(0,0,0,0.03), 0 4px 20px rgba(0,0,0,0.03)",
            border: "1px solid #f3f4f6",
          }}
        >
          <div
          ...
          <InputField
            ...
            onChange={updateInput("balance")}
            prefix="$"
          />
          <InputField
            label="Monthly Contributions"
            value={inputs.monthlyContrib}
            onChange={updateInput("monthlyContrib")}
            prefix="$"
          />
          <InputField
            label="Target Retirement Age"
            value={inputs.retireAge}
            onChange={updateInput("retireAge")}
            suffix="years"
          />
          <InputField
          ...
          {showAdvanced && (
            <div style={{ borderTop: "1px solid #f3f4f6", paddingTop: 18 }}>
              <InputField
                ...
                onChange={updateInput("annualReturn")}
                suffix="%"
                info="10% is the historical S&P 500 average before inflation. After 3% inflation, your real return is ~6.8%. If you want to be more conservative or have a diversified portfolio, use 7-8% instead of 10%."
              />
              <InputField
                label="Expected Inflation"
                value={inputs.inflation}
                onChange={updateInput("inflation")}
                suffix="%"
              ...
              <InputField
                ...
                value={inputs.planAge}
                onChange={updateInput("planAge")}
                suffix="years"
              />
            </div>
          )}
        ...
        {tab === "results" && (
          ...
          >
            <ResultCard
              ...
              value={formatCurrency(results.coastNumber)}
              accent="purple"
              sub="Amount needed today"
              info="The balance you need today so growth alone (no more contributions) reaches your retirement target."
            />
            <ResultCard
              label="Already Coast?"
              value={results.alreadyCoast ? "YES! 🎉" : "Not yet"}
              accent={results.alreadyCoast ? "green" : "default"}
            />
            <div style={{ gridColumn: "1 / -1" }}>
              <ResultCard
                label={`Balance at Retirement Age (${inputs.retireAge})`}
                value={formatCurrency(results.coastProjection)}
                sub={
                  results.coastAge !== null
                    ? `If you stop contributing at Coast FIRE age (${results.coastAge})`
                    : "If you keep investing until retirement"
                }
              />
            </div>
            <ResultCard
              label="Surplus / Shortfall"
              value={formatCurrency(results.surplus)}
              accent={results.surplus >= 0 ? "green" : "red"}
              large
            />
            <ResultCard
              label="Money Lasts Until"
              value={`Age ${results.moneyLastsAge}`}
              accent={
                results.moneyLastsAge === "100+" ||
                results.moneyLastsAge >= inputs.planAge
                  ? "green"
                  : "red"
              }
              large
            ...
            {results.hasShortfallButLasts && (
              <div style={{ gridColumn: "1 / -1" }}>
                ...
                >
                  Good news: your money still lasts at these projected returns.
                  But the 4% rule exists because markets have bad years too.
                  Closing the gap gives you a bigger safety net for when that
                  happens.
                </div>
              </div>
            )}
            {!results.onTrack && (
              <div style={{ gridColumn: "1 / -1" }}>
                <ResultCard
                  ...
                  value={`${formatCurrency(results.extraMonthly)}/mo`}
                  accent="orange"
              ...
              >
                ...
                {[
                  [
                    ...
                    `${(results.realReturn * 100).toFixed(2)}%`,
                  ],
                  ["Years until retirement", results.yearsToRetire],
                  [
                    "Annual contributions",
                    formatCurrency(results.annualContrib),
                  ],
                  [
        ...
        {tab === "chart" && (
          ...
          >
            ...
            >
              {[
                ["#f59e0b", "Coast", false],
                ["#8b5cf6", "Keep Investing (if you don't stop)", true],
                ["#ef4444", "Coast Target", true],
            ...
            {results.noCoast && (
              ...
              >
                Your balance doesn't reach the coast target before retirement,
                but based on projected returns your money still lasts
                {results.moneyLastsAge === "100+"
                  ? " past age 100"
                  : ` until age ${results.moneyLastsAge}`}
                .
              </div>
            )}
          </div>
              ...
              >
                <thead>
                  <tr style={{ background: "#1e1b4b" }}>
                    {["Age", "Balance", "Target", "Withdrawal", "Status"].map(
                      (header) => (
                        <th
                          key={header}
                          style={{
        ...
        <button
          onClick={() => {
            ...
            trackEvent("toggle_howitworks", { open: !showHowItWorks });
          }}
          style={{
            width: "100%",
            marginTop: 16,
        ...
        {showHowItWorks && (
          ...
          >
            {[
              [
                "🧭 A Compass, Not a GPS",
                "This calculator tells you if you're heading in the right direction. When you can ease off the gas and roughly how long your money will last. For the full picture, work with a financial planner.",
              ],
              [
                "📊 Today's Dollars",
                "All values are in today's purchasing power. A 10% return with 3% inflation gives a real return of ~6.8%. Withdrawals are flat because they represent constant purchasing power. $75K always buys the same amount regardless of the year.",
              ],
              [
                "🔥 Coast FIRE Age",
                "This is when your balance crosses the 'Coast Target', the amount that would grow on its own to your retirement goal. After this age, you could stop contributing and still reach your target through growth alone.",
              ],
              [
                "📈 What If I Keep Investing?",
                "The chart shows both paths: what happens if you coast (stop contributing) vs. keep investing. The gap between the lines is the trade-off: more freedom now vs. a bigger cushion later.",
              ],
              [
                "⚠️ Not Included",
                "Taxes: Withdrawals from traditional 401k/IRA are taxed as ordinary income. Roth withdrawals are tax-free. For a rough adjustment, increase your spending by 15-25% to account for taxes.\n\nThis calculator also doesn't include Social Security, pensions, rental income, or other income sources. These would extend how long your money lasts. It also doesn't model portfolio allocation changes over time or market volatility (returns are assumed constant).\n\nWant to see any of these features? Let me know in the feedback below.",
              ],
              ...
              <div key={index} style={{ marginBottom: 16 }}>
                ...
                <div
                  style={{ fontSize: 13, color: "#4b5563", lineHeight: 1.65 }}
                >
                  {body.split("\n\n").map((paragraph, pIndex) => (
                    <p
                      key={pIndex}
                      style={{ margin: pIndex === 0 ? 0 : "10px 0 0" }}
                    >
                      {paragraph}
                    </p>
                  ))}
        ...
        <div style={{ textAlign: "center", padding: "24px 0 8px" }}>
          <p style={{ fontSize: 12, color: "#9ca3af", margin: 0 }}>
            🧭 This calculator is a compass, not a GPS.
          </p>
          <p style={{ fontSize: 11, color: "#b0b8c4", margin: "4px 0 0" }}>
            ...
            <a
              ...
              rel="noopener noreferrer"
              onClick={() =>
                trackEvent("click_affiliate", { location: "footer" })
              }
              style={{
                color: "#7c3aed",
                fontWeight: 600,
